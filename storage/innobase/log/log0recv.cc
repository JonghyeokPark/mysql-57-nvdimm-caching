...
// (jhpark): remove unnecessary part (i.e., UNIV_HOT_BACKUP, UNIV_LSN_DEBUG, etc)
//  we store all log information including block (space, page_no) in our NVM_REDO.

#ifdef UNIV_NVDIMM_CACHE
static
byte*
pmem_recv_apply_log_rec_body(
	mlog_id_t	type,
	byte*		ptr,
	byte*		end_ptr,
	ulint		space_id,
	...
	mtr_t*		mtr)
{
  ...
  fprintf(stderr, "pmem_recv_apply_log_rec_body() is calld ! type :%lu page_id: %lu:%lu\n", type, space_id, page_no);
    ...
    fprintf(stderr, "[DEBU] THIS IS ERROR CASE !!!!!!! STOP!\n");
...
/*===================*/
	mlog_id_t type,              /* mtr log type */
  uint64_t mtr_len,                /* mtr log length */
  uint64_t offset,             /* start offset of real mtr log */
  ...
  buf_block_t*	block)	       /*!< in/out: buffer block */
{
  ...
  // TODO(jhpark): pick appropriate position
  buf = gb_pm_mmap + offset;
...
//				block, &mtr);
        ...
        // TODO(jhpark): check end_lsn !!!
        // how to decide end_lsn? 
        // current NC page ?
        end_lsn = 0;
...
pmem_recv_apply_hashed_log_recs(uint64_t size) {
  ...
  // TODO(jhpark): add mutex ... still needed?
  //mutex_exit(&(recv_sys->mutex));
  // look thru the NVM REDO logs
...
//keep_recovery:
    if (buf_page_peek(page_id)) {
      fprintf(stderr, "[DEBUG] pmem_recv_apply_hashed_log_recs() page exists in buffer ! %lu:%lu\n", page_id.space(), page_id.page_no());
      buf_block_t*	block;
      mtr_start(&mtr);
      ...
      mlog_id_t type;
      const byte *ptr = (gb_pm_mmap + tmp_offset + PMEM_LOG_HDR_SZ);
      type = (mlog_id_t)((ulint)*ptr & ~MLOG_SINGLE_REC_FLAG);

      pmem_recv_recover_page_func(type, recv_mmap_hdr->len, 
                              tmp_offset + PMEM_LOG_HDR_SZ,
                              recv_mmap_hdr->space, recv_mmap_hdr->page_no,
                              block);
      mtr_commit(&mtr);

    } else {
      fprintf(stderr, "[DEBUG] pmem_recv_apply_hashed_log_recs() read page from disk ! %lu:%lu\n", page_id.space(), page_id.page_no());
      recv_read_in_area(page_id);
 //     goto keep_recovery;

      // read page from disk directly !!!! refer from recv_read_in_area() function
      //dberr_t err;
      //buf_read_page_low(&err, true, 0, BUF_READ_ANY_PAGE, page_id, page_size, true);
    }
